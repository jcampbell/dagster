...
Dagster solids have [inputs and outputs](/concepts/solids-pipelines/solids#solid-inputs-and-outputs). When a solid produces an output, what happens to it? When a solid needs an input, how is it loaded?
...
IOManagers are [resources](/concepts/modes-resources-presets/modes-resources), which means users can supply different IOManagers for the same solid outputs in different situations. For example, you might use an in-memory IOManager for unit-testing a pipeline and an S3IOManager in production.
...
```python file=/concepts/io_managers/default_io_manager.py
...
```python file=/concepts/io_managers/io_manager_per_output.py startafter=start_marker endbefore=end_marker
...
```python file=/concepts/io_managers/output_config.py startafter=io_manager_start_marker endbefore=io_manager_end_marker
class MyIOManager(IOManager):
    def handle_output(self, context, obj):
...
```python file=/concepts/io_managers/output_config.py startafter=execute_start_marker endbefore=execute_end_marker
...
```python file=/concepts/io_managers/metadata.py startafter=solids_start_marker endbefore=solids_end_marker
...
```python file=/concepts/io_managers/metadata.py startafter=io_manager_start_marker endbefore=io_manager_end_marker
class MyIOManager(IOManager):
    def handle_output(self, context, obj):
...
```python file=/concepts/io_managers/root_input_manager.py startafter=start_marker endbefore=end_marker
...
```python file=/concepts/io_managers/config_input_manager.py startafter=def_start_marker endbefore=def_end_marker
...
```python file=/concepts/io_managers/config_input_manager.py startafter=execute_start_marker endbefore=execute_end_marker
...
```python file=/concepts/io_managers/subselection.py startafter=start_marker endbefore=end_marker
@root_input_manager(input_config_schema={"table_name": str})
...
```python file=/concepts/io_managers/subselection.py startafter=start_execute_subselection endbefore=end_execute_subselection
execute_pipeline(
        my_pipeline,
